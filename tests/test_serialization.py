...
def make_acquisition():
    ...
    # Build data arrays
    trajectory = np.ones((2, 4), dtype='float32')
    data = np.ones((2, 4), dtype='complex64')
...
def make_image():
    ...
    header.user_int = [0] * 8
    header.user_float = [0.0] * 8
    header.attribute_string_len = 6
    # Build data array
    data = np.ones((1, 2, 2, 2), dtype='float32')
    attribute_string = "foobar"
    # Construct Image
    img = Image(header, data, attribute_string)
    return img
...
def test_acquisition_serialization():
    ...
    assert np.allclose(acq.trajectory, acq2.trajectory)
...
def test_image_serialization():
    ...
    assert img.header.matrix_size == img2.header.matrix_size
    assert img.header.channels == img2.header.channels
...
def test_waveform_serialization():
    ...
    assert wf.header.number_of_samples == wf2.header.number_of_samples
    assert wf.header.channels == wf2.header.channels
    assert wf.header.waveform_id == wf2.header.waveform_id
